...
// 2. 根据基数生成全范围标准阻值函数 (从 1Ω 到 10MΩ)
export function generateResistorValues(baseSeries: number[]): number[] {
    const multipliers = [1, 10, 100, 1000, 10000, 100000, 1000000];
    const fullList: number[] = [];
    for (const m of multipliers) {
        for (const base of baseSeries) {
            // 保持精度处理
            fullList.push(Number((base * m).toFixed(2)));
        }
    }
    return fullList;
}
