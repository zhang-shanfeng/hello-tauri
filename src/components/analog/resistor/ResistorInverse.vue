...
import { HelpCircleIcon, CheckCircle2Icon } from "@lucide/vue";
// 假设你之前提取的常量基数库
...
const matchResults = computed(() => {
    ...
    if (count === 2) {
        ...
        const sortedByProximity = [...validCandidates].sort(
            (a, b) =>
                Math.abs(a - targetSingleEst) - Math.abs(b - targetSingleEst),
        );
        const optimizedCandidates = sortedByProximity
            .slice(0, 120)
            .sort((a, b) => a - b); // 限制在120个高概率阻值内
                    ...
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                <template
                                    v-for="(r, rIdx) in result.resistors"
                                    :key="rIdx"
                                >
                                    <span
                                    ...
                                    <span
                                        v-if="
                                            rIdx < result.resistors.length - 1
                                        "
                                        class="text-zinc-500 dark:text-zinc-400 text-lg font-bold"
                                        >//</span
                                    >
                                </template>
                            </div>
                        ...
                        <div
                            class="flex items-center gap-4 mt-3 sm:mt-0 justify-between sm:justify-end text-sm"
                        >
                            <span
                                class="text-zinc-700 dark:text-zinc-300 font-medium"
                            >
                                并联总值:
                                <strong class="font-mono text-lg">{{
                                    formatResistanceSimple(result.actual)
                                }}</strong>
                            </span>
                            <span
                                :class="[
